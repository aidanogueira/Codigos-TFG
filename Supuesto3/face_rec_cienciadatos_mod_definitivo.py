...
def detectar_caras(imagen: Union[PIL.Image.Image, np.ndarray],
                   ...
                   keep_all: bool        = True,
                   min_face_size: int    = 10, # incialmente 20
                   thresholds: list      = [0.6, 0.7, 0.7],
                   device: str           = None,
                   min_confidence: float = 0.5, #incialmente 0.5
                   fix_bbox: bool        = True,
...
def identificar_caras(embeddings: np.ndarray,
                      dic_referencia: dict,
                      threshold_similaridad: float = 0.1) -> list:
    # inicialmente 0.3
    """
...
def obtener_hora_foto(imagen_path):
    try:
        ...
        if exif is not None:
            # Aquí procesar los datos EXIF si existen
            hora_foto = exif.get(36867)  # 36867 es la etiqueta para DateTimeOriginal
            return hora_foto
        else:
        ...
        print(f"Error accessing EXIF data: {e}")
        return None
...
directory = 'Dataset'
save_directory = 'PositivosDoble'
image_files = [f for f in os.listdir(directory) if f.endswith(('.jpg', '.jpeg', '.png', '.tif', '.JPG', '.PNG', '.JPEG', '.bmp'))]
...
for image_file in image_files:
    ...
    detection = pipeline_deteccion_imagen(
            imagen = imagen,
            dic_referencia        = dic_referencias,
            min_face_size         = 20,
        ...
        if save:
            ...
            if hora_foto is not None:
                ...
                guardar_imagen_cv2(imagen=imagen_bbox, 
                                   ...
                                   nombre_archivo=hora_foto.replace(':', '.') + image_file + '_' + ext,
                                   texto=texto)
            else:
